...
const InsuredPersonsValidationSchema = yup.object().shape({
    ...
    personal_identification_number: yup
        ...
        .test("identification-number-validation", function (value) {
            ...
            // Debug: log the type to see what we're working with
            // console.log('Type object:', type, 'Label:', type?.label, 'Value:', type?.value);
            ...
            if (regex) {
                if (!regex.test(value)) {
                    const errorMessage =
                        getIdentificationNumberErrorMessage(type);
                    // console.log('Regex failed, error message:', errorMessage);
